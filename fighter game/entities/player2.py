...
class Player():
    ...
    def attack(self, target, player_data):
        self.attacking = True
        atttacking_rect = pygame.Rect(((self.rect.left - self.rect.width/2) if self.face_foward else self.rect.centerx), self.rect.y, self.rect.width, self.rect.height)
        if atttacking_rect.colliderect(target.rect):
            #print("player 2 hit player 1")
            player_data["p1_character"][1] -= 10
    ...
    def move(self, target, player_data):
        ...
        if self.attacking == False:
            ...
            if key[pygame.K_i] and self.jump == False:
                self.vel_y = -30
                self.jump = True
            elif key[pygame.K_i] and self.jump == True and self.double_jump == False and self.rect.bottom < 300:
                self.vel_y = -20
